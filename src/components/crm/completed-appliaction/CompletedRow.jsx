...
import { Ban, Calendar, Clock, Mail } from "lucide-react";
...
const CompletedRow = ({
  ...
  const iconAccent = `h-4 w-4 ${textAccent}`;
    ...
    >
      ...
      <td className={cellStyle}>
        <CallButton
          ...
          size="small"
          variant="default"
          className="px-5 py-2 rounded-md text-sm font-semibold border transition-all duration-200 hover:scale-105"
        />
      </td>
      <td className={cellStyle}><span className={`text-sm ${textSecondary}`}>{application.enquirySource || "N/A"}</span></td>
      <td className={cellStyle}>
        <CRNLink crnNo={application.crnNo} userId={application.id} />
      </td>
      <td className={cellStyle}>
      ...
      <td className={cellStyle}>
        <div className="flex items-center justify-center gap-2">
          <Calendar className={iconAccent} />
          <span className={`text-sm ${textSecondary}`}>{application.completeDate}</span>
        </div>
      </td>
      <td className={cellStyle}>
        <div className="flex items-center justify-center gap-2">
          <Clock className={iconAccent} />
          <span className={`text-sm ${textSecondary}`}>{application.completeTime}</span>
        </div>
      </td>
      ...
      <td className={cellStyle}>
        <button
          ...
          className={`rounded px-3 py-1 text-xs font-medium ${
            isDark
              ? "border border-green-800 bg-green-200 text-green-950"
      ...
      <td className={cellStyle}>
        <BlacklistButton
          ...
          isDark={isDark}
          onBlacklist={() => onBlacklist?.(application.userId)}
        />
      </td>
    </tr>
  );
