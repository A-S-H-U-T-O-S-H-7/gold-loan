...
const BankStatementDocument = ({ 
  ...
  if (!hasDoc || !fileName) {
    return (
      <div 
        ...
        title={`Bank Statement Missing: ${fileName || 'No file'}`}
      >
  ...
  return (
    <Link
      href={`/documents/view?file=${encodeURIComponent(fileName)}&type=bank_statement${applicationId ? `&appId=${applicationId}` : ''}`}
      target="_blank"
