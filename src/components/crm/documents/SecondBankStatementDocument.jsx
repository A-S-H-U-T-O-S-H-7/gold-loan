...
const SecondBankStatementDocument = ({ 
  ...
  if (!hasDoc || !fileName) {
    return (
      <div 
        ...
        title={`Second Bank Statement Missing: ${fileName || 'No file'}`}
      >
  ...
  return (
    <Link
      href={`/documents/view?file=${encodeURIComponent(fileName)}&type=second_bank_statement${applicationId ? `&appId=${applicationId}` : ''}`}
      target="_blank"
      ...
      title={`View Second Bank Statement`}
    >
      <FileText 
        size={iconSizes[size]} 
