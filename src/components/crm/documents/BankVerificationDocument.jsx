...
const BankVerificationDocument = ({ 
  ...
  if (!hasDoc || !fileName) {
    return (
      <div 
        ...
        title={`Bank Verification Report Missing: ${fileName || 'No file'}`}
      >
  ...
  return (
    <Link
      href={`/documents/view?file=${encodeURIComponent(fileName)}&type=bank_verif_report${applicationId ? `&appId=${applicationId}` : ''}`}
      target="_blank"
