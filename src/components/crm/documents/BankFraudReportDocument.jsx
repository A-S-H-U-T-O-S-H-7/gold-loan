...
const BankFraudReportDocument = ({ 
  ...
  if (!hasDoc || !fileName) {
    return (
      <div 
        ...
        title={`Bank Fraud Report Missing: ${fileName || 'No file'}`}
      >
  ...
  return (
    <Link
      href={`/documents/view?file=${encodeURIComponent(fileName)}&type=bank_fraud_report${applicationId ? `&appId=${applicationId}` : ''}`}
      target="_blank"
